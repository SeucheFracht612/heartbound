...
int main(int argc, char** argv) {
    ...
    for (const auto& fingerprint : report.mod_fingerprints) {
        core::log(core::LogLevel::info, "  " + fingerprint.id + " prototypes=" +
                                            std::to_string(fingerprint.prototype_count) +
    ...
    core::log(core::LogLevel::info,
              "  data-update patches=" +
                  std::to_string(
                      count_patch_stage(report, modding::GenericPrototypePatchStage::data_update)) +
                  ", final-fix patches=" +
                  std::to_string(
                      count_patch_stage(report, modding::GenericPrototypePatchStage::final_fix)));
    ...
    core::log(core::LogLevel::info, "Room descriptor definitions: " +
    ...
    core::log(
        core::LogLevel::info,
        "Material asset references: " + std::to_string(report.material_assets.references.size()) +
