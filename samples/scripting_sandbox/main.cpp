...
int main() {
    ...
    sandbox_ready_api.required_permissions.push_back(scripting::ScriptPermission::emit_commands);

    scripting::ScriptRuntimeDesc runtime_desc{scripting::ScriptBackend::luau, 4096, 32, 8, 1024};
    ...
    if (!scripting::script_module_has_permission(*info,
        ...
        core::log(core::LogLevel::error, "script module is missing expected read_prototypes grant");
        return 1;
    }
    if (!scripting::script_module_has_permission(*info,
                                                 scripting::ScriptPermission::emit_commands)) {
        core::log(core::LogLevel::error, "script module is missing expected emit_commands grant");
        return 1;
    }
    ...
    core::log(core::LogLevel::info, "Script call returned " +
                                        call.value().return_value.string_value + " through " +
    ...
    if (!event_call || event_call.value().emitted_events.size() != 1) {
        core::log(core::LogLevel::error, event_call
                                             ? "script event call returned unexpected event count"
