...
int main() {
    ...
    auto client_welcome = client_session.receive_server_message(welcome_messages.value().front());
    if (!client_welcome) {
    ...
    auto decoded_packet =
        net::TransportPacketCodec::decode(outgoing_packet, net::TransportPacketCodecConfig{1024});
    if (!decoded_packet) {
    ...
    if (tick.value().command_reports.size() != 1 || !tick.value().command_reports.front().success) {
    ...
    core::log(core::LogLevel::info,
              "Command " + tick.value().command_reports.front().command_type + " sequence " +
                  ...
                  " after result event count " +
                  std::to_string(command_results.front().event_count) + " through host session " +
                  std::string(net::host_session_state_name(host.state())));
    return 0;
}
