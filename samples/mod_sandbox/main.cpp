...
int main() {
    ...
    for (const auto& prototype : registry.prototypes_of_kind(modding::PrototypeKinds::scenario)) {
    ...
    core::log(
        core::LogLevel::info,
        "Material asset references: " + std::to_string(material_asset_report.references.size()) +
            ", overrides=" + std::to_string(material_asset_report.override_count()));

    core::log(core::LogLevel::info, "Registered prototypes: " + std::to_string(registry.size()));
    core::log(core::LogLevel::info,
    ...
    if (clay_voxel_texture) {
        core::log(core::LogLevel::info,
                  "Resolved clay material asset " + clay_voxel_texture.value().generic_string());
    } else {
