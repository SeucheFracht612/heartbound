...
int main() {
    ...
    if (!process || !state.processes().insert(std::move(process).value())) {
        core::log(core::LogLevel::error, "failed to insert process");
        return 1;
    }
    ...
    core::log(core::LogLevel::info, "Build objects: " + std::to_string(stats.build_object_count));
    ...
    core::log(core::LogLevel::info,
              "Snapshot build objects: " + std::to_string(snapshot.value().build_pieces.size()));
    core::log(core::LogLevel::info,
