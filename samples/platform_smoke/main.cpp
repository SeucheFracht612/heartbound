...
int main() {
    ...
    core::log(
        ...
        "Native platform contract: windows=" +
            ...
            ", text_input=" + std::string(native_capabilities.supports_text_input ? "yes" : "no") +
            ", mouse_input=" +
            std::string(native_capabilities.supports_mouse_input ? "yes" : "no") + ", displays=" +
            ...
            ", clipboard=" + std::string(native_capabilities.supports_clipboard ? "yes" : "no") +
    ...
    for (const auto& display : displays) {
        auto message = "Display " + std::to_string(display.index) + " " + display.name + " " +
                       std::to_string(display.width_px) + "x" + std::to_string(display.height_px) +
                       " at " + std::to_string(display.x_px) + "," + std::to_string(display.y_px);
    ...
    queued =
        platform.value()->queue_event(platform::PlatformEvent{platform::PlatformEventKind::key_down,
                                                              window.value(),
    ...
    auto run_status = platform::run_platform_app(
        *platform.value(), platform::AppRunConfig{8},
        [window_id](platform::IPlatform& active_platform, std::uint64_t frame,
            ...
            if (const auto snapshot = active_platform.input_snapshot(window_id);
                snapshot &&
                active_platform.was_mouse_button_pressed(window_id, platform::MouseButton::left)) {
                core::log(
                    core::LogLevel::info,
                    "Mouse " +
            ...
            core::log(core::LogLevel::debug,
                      "Platform frame " + std::to_string(frame) + " at " + std::to_string(now_ms));
