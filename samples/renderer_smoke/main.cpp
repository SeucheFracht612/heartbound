...
int main(int argc, char** argv) {
    return heartstead::core::run_no_argument_process_entry(argc, argv, [] {
        ...
        core::log(core::LogLevel::info,
                  "Vulkan backend status: " + std::string(vulkan_info.status));
        ...
        core::log(
            ...
            "Vulkan contract: surface=" +
                ...
                ", gpu=" +
                std::string(vulkan_caps.requires_gpu_device ? "required" : "not_required") +
        ...
        if (desc.backend == renderer::rhi::RenderBackend::vulkan) {
            const auto native_info =
                platform::platform_backend_info(platform::PlatformBackend::native);
        ...
        core::log(core::LogLevel::info,
                  "Renderer capabilities: present=" +
                      std::string(capabilities.supports_present ? "yes" : "no") + ", validation=" +
                      std::string(capabilities.supports_validation ? "yes" : "no") +
                      ", shader_modules=" +
                      std::string(capabilities.supports_shader_modules ? "yes" : "no") +
                      ", debug_markers=" +
                      std::string(capabilities.supports_debug_markers ? "yes" : "no") +
        ...
        core::log(core::LogLevel::info, "Registered " + std::to_string(material_registry.size()) +
                                            " material definition(s) for " +
                                            std::string(renderer::materials::material_domain_name(
                                                debug_material.domain)));
        ...
        auto index_upload = device.value()->upload_buffer(
            renderer::rhi::RenderBufferDesc{renderer::rhi::RenderBufferUsage::index,
                                            index_bytes.size(), "debug_chunk_indices"},
            index_bytes);
        if (!index_upload) {
        ...
        core::log(core::LogLevel::info,
                  "Uploaded chunk mesh buffers: vertices=" +
                      std::to_string(vertex_upload.value().byte_size) + " bytes, indices=" +
                      std::to_string(index_upload.value().byte_size) + " bytes, gpu_backed=" +
                      std::string(vertex_upload.value().gpu_backed ? "yes" : "no"));
        ...
        renderer::rhi::RenderGraphicsPipelineDesc graphics_desc{
        ...
        core::log(core::LogLevel::info,
                  "Bound " + std::to_string(draw_stats.value().draw_count) + " mesh draw(s) with " +
                      std::to_string(draw_stats.value().material_count) +
                      " material(s), submitted=" +
        ...
        if (device.value()->backend() == renderer::rhi::RenderBackend::vulkan) {
            ...
            core::log(core::LogLevel::info,
                      "Executed supported Vulkan frame plan with " +
                          std::to_string(planned_stats.value().render_pass_count) + " pass(es)");
        } else {
            ...
            core::log(core::LogLevel::info,
                      "Executed renderer frame plan with " +
                          std::to_string(planned_stats.value().render_pass_count) + " pass(es)");
        }
        ...
        const auto release_uniform =
            device.value()->release_resource(uniform_upload.value().handle);
        const auto release_texture =
            device.value()->release_resource(texture_upload.value().handle);
        const auto release_vertices =
            device.value()->release_resource(vertex_upload.value().handle);
        ...
        core::log(core::LogLevel::info,
                  "Renderer smoke completed " +
                      std::to_string(device.value()->completed_frame_count()) + " frame(s)");
        return 0;
    });
}
